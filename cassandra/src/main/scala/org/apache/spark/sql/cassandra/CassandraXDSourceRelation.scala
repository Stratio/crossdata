...
class CassandraXDSourceRelation(tableRef: TableRef,
    ...
    udfs.get(strId) map { udf =>
      val actualParams = udf.children.collect {
        case at: AttributeReference if udfs contains at.toString =>
          Left(resolveUDFsReferences(at.toString(), udfs).get)
  ...
  private def filterToCqlAndValue(filter: Any,
    ...
    filter match {
      ...
      case sources.GreaterThanOrEqual(attribute, f: AttributeReference)
...
object CassandraXDSourceRelation {
  ...
  def apply(tableRef: TableRef,
    ...
    new CassandraXDSourceRelation(tableRef = tableRef,
