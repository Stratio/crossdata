...
class CassandraInsertTableIT extends CassandraInsertCollection {
  ...
  it should "insert a row using INSERT INTO table VALUES in Cassandra" in {
    _xdContext
      .sql(s"INSERT INTO $Table VALUES (20, 25, [(x -> 3)], ['proof'], 'proof description', true, (x->[1]), (a->2), 'Eve' )")
  ...
  it should "insert a row using INSERT INTO table(schema) VALUES in Cassandra" in {
    _xdContext
      .sql(s"INSERT INTO $Table(id, age, name) VALUES (21, 25, 'Peter')")
      .collect() should be(Row(1) :: Nil)
  ...
  it should "insert multiple rows using INSERT INTO table VALUES in Cassandra" in {
    ...
    //EXPECTATION
    val results = sql(
        s"select id,age,comment,enrolled,name,array_test,map_test,array_map,map_array from $Table where id=22 or id=23 or id=24")
      .collect()
  ...
  it should "insert multiple rows using INSERT INTO table(schema) VALUES in Cassandra" in {
    _xdContext
      .sql(s"INSERT INTO $Table(id, age, name, enrolled) VALUES ( 25, 50, 'Samantha', true),( 26, 1, 'Charlie', false)")
