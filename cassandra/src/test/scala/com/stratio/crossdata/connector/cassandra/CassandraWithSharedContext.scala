...
trait CassandraWithSharedContext
  ...
  abstract override def saveTestData: Unit = {
    ...
    def stringifySchema(schema: Map[String, String]): String =
      schema.map(p => s"${p._1} ${p._2}").mkString(", ")
    ...
    def parseElement(element: Any): String = {
      element match {
        case map: Map[_, _] =>
          map map {
            case (key, value) =>
              s"${parseElement(key)} : ${parseElement(value)}"
          } mkString ("{", ", ", "}")
        case list: Seq[_] =>
  ...
  override protected def cleanTestData: Unit =
    client.get._2.execute(s"DROP KEYSPACE $Catalog")
