...
object CrossdataStreamingHelper extends SparkLoggerComponent {
      ...
      Try {
        ...
        ephemeralTable.options.outputFormat match {
          case EphemeralOutputFormat.JSON =>
            saveToKafkaInJSONFormat(dataFrame, topic, kafkaOptionsMerged)
          case _ =>
            saveToKafkaInRowFormat(dataFrame, topic, kafkaOptionsMerged)
        }
      } match {
  ...
  private[streaming] def toWindowDStream(
