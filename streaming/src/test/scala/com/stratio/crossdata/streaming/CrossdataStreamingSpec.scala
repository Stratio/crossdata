...
class CrossdataStreamingSpec extends BaseStreamingXDTest with CommonValues {
  ...
  "CrossdataStreaming" should "return a empty Sparkconf according to the table options" in {
    val XDStreaming =
      new CrossdataStreaming(TableName, Map.empty[String, String], Map.empty[String, String])
    val configuration =
      XDStreaming.configToSparkConf(ephemeralTableModelWithoutSparkOptions).getAll
  ...
  "CrossdataStreaming" should "return Sparkconf according to the table options with prefix" in {
    val XDStreaming =
      new CrossdataStreaming(TableName, Map.empty[String, String], Map.empty[String, String])
    val configuration =
      XDStreaming.configToSparkConf(ephemeralTableModelWithSparkOptionsPrefix).getAll
    val expected = Array(("spark.defaultParallelism", "50"))
