...
class HazelcastSessionProviderSpec extends SharedXDContextTest {
  ...
  "HazelcastSessionProvider" should "provides new sessions whose properties are initialized properly" in {
    ...
    tempCatalogs.head
      .asInstanceOf[XDTemporaryCatalogWithInvalidation]
      .underlying shouldBe a[HashmapCatalog]
  ...
  it should "provides a common persistent catalog and isolated catalogs" in {
    ...
    val hazelcastSessionProvider =
      new HazelcastSessionProvider(xdContext.sc, ConfigFactory.empty())
  ...
  def testInvalidation(testDescription: String)(
    ...
    it should testDescription in {
      ...
      val hazelcastSessionProviderA =
        new HazelcastSessionProvider(xdContext.sc, ConfigFactory.empty())
      val hazelcastSessionProviderB =
        new HazelcastSessionProvider(xdContext.sc, ConfigFactory.empty())
