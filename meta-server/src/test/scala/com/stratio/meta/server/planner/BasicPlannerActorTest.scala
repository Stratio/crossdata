...
import com.stratio.meta.server.actors.{PlannerActor, TimeTracker, ExecutorActor}
...
with DefaultTimeout with FunSuiteLike with BeforeAndAfterAll
{
  ...
  val plannerRef = system.actorOf(PlannerActor.props(executorRef,engine.getPlanner),"TestPlanerActor")
  val plannerRefTest= system.actorOf(PlannerActor.props(testActor,engine.getPlanner),"TestPlanerActorTest")
  ...
  def proccess (msg:String) : (String) ={
    ...
    val futureExecutorResponse=plannerRef.ask(stmt1)(2 second)
    try{
