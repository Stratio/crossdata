...
with DefaultTimeout with FunSuiteLike with BeforeAndAfterCassandra
{
  ...
  test ("queryActor use KS not create"){
    ...
    within(3000 millis){
      ...
      assertEquals(querying.proccess(msg,queryRef,engine,4),"Keyspace 'ks_demo_not' does not exist" )
    }
  }
  test ("queryActor insert into table not create yet without error"){
    ...
    within(3000 millis){
      ...
      assertEquals(querying.proccess(msg,queryRef,engine,4),"unconfigured columnfamily demo" )
    }
  }
  ...
  test ("queryActor create table  create yet"){
    ...
    within(3000 millis){
      ...
      val msg="create TABLE demo (field1 text PRIMARY KEY , field2 text);"
      assertEquals(querying.proccess(msg,queryRef,engine,4),"Table ks_demo.demo already exists" )
    }
  }
  ...
  test ("queryActor drop KS  not exit"){
    ...
    within(3000 millis){
      ...
      val msg="drop keyspace ks_demo ;"
      assertEquals(querying.proccess(msg,queryRef,engine,4),"Cannot drop non existing keyspace 'ks_demo'." )
    }
  }
