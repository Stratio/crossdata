 ...
 override def beforeCassandraFinish() {
  ...
  test ("QueryActor use KS not create"){
    ...
    within(3000 millis){
      ...
      assertEquals(querying.proccess(msg,executorRef,engine,1),"Keyspace 'ks_demo_not' does not exist" )
    }
  }
  test ("QueryActor insert into table not create yet without error"){
    ...
    within(3000 millis){
      ...
      assertEquals(querying.proccess(msg,executorRef,engine,1),"unconfigured columnfamily demo" )
    }
  }
  ...
  test ("QueryActor create table  create yet"){
    ...
    within(3000 millis){
      ...
      val msg="create TABLE demo (field1 text PRIMARY KEY , field2 text);"
      assertEquals(querying.proccess(msg,executorRef,engine,1),"Table ks_demo.demo already exists" )
    }
  }
  ...
  test ("QueryActor drop KS  not exit"){
    ...
    within(3000 millis){
      ...
      val msg="drop keyspace ks_demo ;"
      assertEquals(querying.proccess(msg,executorRef,engine,1),"Cannot drop non existing keyspace 'ks_demo'." )
    }
  }
