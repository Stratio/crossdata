...
trait ServerActorTest extends ActorReceiveUtils with FunSuiteLike with MockFactory with ServerConfig {
  ...
  val metadataParsedQuery1 = new MetadataParsedQuery(new BaseQuery(incQueryId(), "create table "+tableName+";",
    new CatalogName(catalogName)),
    metadataStatement1)
  ...
  val columnNme= new ColumnName(catalogName,tableName,columnName)
  ...
  def initializeTablesInfinispan(): TableMetadata = {
    ...
    metadataManager.createTestTable(new ClusterName(myclusterName), catalogName,"table1", Array("name", "age"),
