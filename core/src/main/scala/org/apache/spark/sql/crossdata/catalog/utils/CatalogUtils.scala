...
object CatalogUtils extends Logging {
  ...
  protected[crossdata] def externalCatalog(catalystConf: CatalystConf,
  ...
  protected[crossdata] def streamingCatalog(catalystConf: CatalystConf,
    ...
    if (serverConfig.hasPath(CoreConfig.StreamingCatalogClassConfigKey)) {
      ...
      val constr: Constructor[_] =
        xdStreamingCatalog.getConstructor(classOf[CatalystConf], classOf[Config])
