...
private[sql] object XDDataFrame {
  ...
  def findNativeQueryExecutor(
    ...
    val leafs = optimizedLogicalPlan.collect {
    ...
    if (!allLeafsAreNative(leafs)) {
      ...
      val nativeExecutors: Seq[NativeScan] = leafs.map {
        case LogicalRelation(ns: NativeScan, _) => ns
      }
...
class XDDataFrame private[sql] (
  ...
  override def collect(): Array[Row] = {
    ...
    if (sqlContext.cacheManager.lookupCachedData(this).nonEmpty) {
      ...
      val nativeQueryExecutor: Option[NativeScan] = findNativeQueryExecutor(
  ...
  def flattenedCollect(): Array[Row] = {
    ...
    def flatRows(
      ...
      require(
          firstLevelNames.isEmpty || rows.isEmpty || firstLevelNames.size == rows.headOption
            .map(_.length)
            .getOrElse(0))
      ...
      rows map {
        case row: GenericRowWithSchema =>
          ...
          val horizontallyFlattened: Row =
            new GenericRowWithSchema(newFieldsArray.map(_._2),
                                     StructType(newFieldsArray.map(_._1)))
    ...
    def verticallyFlatRowArrays(row: GenericRowWithSchema)(
      ...
      cartesian(arrayColumnValues).take(limit) map {
        case replacements: Seq[(Int, _) @unchecked] =>
          val idx2newVal: Map[Int, Any] = replacements.toMap
          val values = elementsWithIndex map {
            case (prevVal, idx: Int) =>
    ...
    queryExecution.optimizedPlan match {
      case Limit(lexp, Project(plist, child)) =>
        processProjection(plist, child, lexp.toString().toInt)
      case Project(plist, child) => processProjection(plist, child)
      case Limit(lexp, _) =>
        iterativeFlatten(collect())(lexp.toString().toInt) toArray
  ...
  override def limit(n: Int): DataFrame =
    XDDataFrame(sqlContext, Limit(Literal(n), logicalPlan))
  ...
  private[this] def executeNativeQuery(
    ...
    val planSupported = !containsSubfields && queryExecution.optimizedPlan
        .map(lp => lp)
        .forall(provider.isSupported(_, queryExecution.optimizedPlan))
    if (planSupported) {
  ...
  private[this] def notSupportedProject(
    ...
    optimizedLogicalPlan collectFirst {
      ...
      case a @ Project(seq, _) if seq.collectFirst {
        ...
        a
      case a @ Project(seq, _) if seq.collectFirst {
            case Alias(b: GetMapValue, _) => a
          }.isDefined =>
        a
      case a @ Project(seq, _) if seq.collectFirst {
            case Alias(b: GetStructField, _) => a
          }.isDefined =>
        a
    } isDefined
  }
  ...
  private[crossdata] object WithTrackerFlatMapSeq {
    implicit def seq2superflatmapseq[T](s: Seq[T]): WithTrackerFlatMapSeq[T] =
      new WithTrackerFlatMapSeq(s)
  }
