...
object XDCatalog extends CrossdataSerializer {
  ...
  implicit def asXDCatalog(catalog: Catalog): XDCatalog =
    catalog.asInstanceOf[XDCatalog]
  ...
  case class IndexIdentifier(indexType: String, indexName: String) {
    ...
    def asTableIdentifier: TableIdentifier =
      TableIdentifier(indexType, Option(indexName))
  }

  case class CrossdataTable(tableIdentifier: TableIdentifierNormalized,
  ...
  def deserializeUserSpecifiedSchema(schemaJSON: String): StructType =
    read[StructType](schemaJSON)

  def serializePartitionColumn(partitionColumn: Array[String]): String =
    write(partitionColumn)

  def deserializePartitionColumn(partitionColumn: String): Array[String] =
    read[Array[String]](partitionColumn)

  def serializeOptions(options: Map[String, String]): String = write(options)

  def deserializeOptions(optsJSON: String): Map[String, String] =
    read[Map[String, String]](optsJSON)
...
trait XDCatalog
  ...
  def registerView(viewIdentifier: ViewIdentifier,
