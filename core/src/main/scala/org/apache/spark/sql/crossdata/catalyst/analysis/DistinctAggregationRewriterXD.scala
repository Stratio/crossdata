...
import org.apache.spark.sql.catalyst.expressions.{Alias, AttributeReference, Coalesce, EqualTo, Expression, If, Literal, NamedExpression, aggregate}
...
case class DistinctAggregationRewriter(conf: CatalystConf) extends Rule[LogicalPlan] {
  ...
  private def aggregateExpressionContainsAlias(aggregateExpressions: Seq[NamedExpression]): Boolean = {
    aggregateExpressions.exists {
      case UnresolvedAlias(_) => true
      case _ => false
    }
  }

  def apply(plan: LogicalPlan): LogicalPlan = plan transformUp  {
    case a: Aggregate if !aggregateExpressionContainsAlias(a.aggregateExpressions) => rewrite(a)
  }
