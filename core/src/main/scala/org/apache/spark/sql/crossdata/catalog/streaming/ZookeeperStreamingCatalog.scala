...
class ZookeeperStreamingCatalog(val catalystConf: CatalystConf, serverConfig: Config)
  ...
  private[spark] val ephemeralQueriesDAO = new EphemeralQueriesTypesafeDAO(
      streamingConfig.getConfig(CoreConfig.CatalogConfigKey))
  private[spark] val ephemeralTableStatusDAO = new EphemeralTableStatusTypesafeDAO(
      streamingConfig.getConfig(CoreConfig.CatalogConfigKey))
  ...
  override def createEphemeralTable(
    ...
    if (!existsEphemeralTable(ephemeralTable.name)) {
      createEphemeralStatus(
          ephemeralTable.name,
          EphemeralStatusModel(ephemeralTable.name, EphemeralExecutionStatus.NotStarted))
  ...
  override def dropEphemeralTable(tableIdentifier: String): Unit = {
    ...
    ephemeralQueriesDAO.dao.getAll().filter(_.ephemeralTableName == tableIdentifier) foreach {
      query =>
