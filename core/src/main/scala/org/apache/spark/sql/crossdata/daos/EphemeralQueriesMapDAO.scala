...
trait EphemeralQueriesMapDAO extends DAOComponent[String, Array[Byte], EphemeralQueryModel]
with ZookeeperRepositoryComponent with MapConfigComponent with SparkLoggerComponent with CrossdataSerializer {
  ...
  trait StreamingQueriesDAO extends DAO with CrossdataSerializer{

    def fromVtoM[TM >: EphemeralQueryModel <: EphemeralQueryModel : Manifest](v: Array[Byte]): TM =
      read[TM](new String(v))

    def fromMtoV[TM <: EphemeralQueryModel : Manifest](m: TM): Array[Byte] =
      write(m).getBytes

    def entity : String = EphemeralQueriesPath
  }
