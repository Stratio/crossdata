...
private[sql] object ExtendedDataSourceStrategy extends Strategy with SparkLoggerComponent {
  ...
  def apply(plan: LogicalPlan): Seq[execution.SparkPlan] = plan match {
    ...
    case ExtendedPhysicalOperation(projects,
                                   filters,
                                   l @ LogicalRelation(t: NativeFunctionExecutor, _))
      ...
      pruneFilterProjectUdfs(
          ...
          (requestedColumns, srcFilters, attr2udf) =>
            toCatalystRDD(l, requestedColumns, t.buildScan(requestedColumns.map {
              case nat: AttributeReference if attr2udf contains nat.toString =>
                nat.toString
              case att => att.name
  ...
  protected def pruneFilterProjectUdfs(
      plan: LogicalPlan,
      relation: LogicalRelation,
      projects: Seq[NamedExpression],
      filterPredicates: Seq[Expression],
      scanBuilder: (Seq[Attribute], Array[Filter], Map[String, NativeUDF]) => RDD[InternalRow]) = {
    import org.apache.spark.sql.sources.CatalystToCrossdataAdapter

    val (pro, fil, att2udf) = (CatalystToCrossdataAdapter
      .getConnectorLogicalPlan(plan, projects, filterPredicates): @unchecked) match {
    ...
    val filterSet = AttributeSet(
        filterPredicates.flatMap(
            _.references flatMap {
          case nat: AttributeReference if att2udf contains nat =>
            CatalystToCrossdataAdapter.udfFlattenedActualParameters(nat, (x: Attribute) => x)(
                att2udf) :+ nat
    ...
    val scan = execution.PhysicalRDD
      .createFromDataSource(requestedColumns, scanBuilder(requestedColumns, fil, att2udf map {
  ...
  private def cannotExecuteNativeUDF(udfsIgnored: Seq[AttributeReference]) =
    throw new AnalysisException(
        "Some filters containing native UDFS cannot be executed on the datasource." +
