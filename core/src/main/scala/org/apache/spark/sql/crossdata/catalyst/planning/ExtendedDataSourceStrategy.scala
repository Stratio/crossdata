...
private[sql] object ExtendedDataSourceStrategy
    ...
    case ExtendedPhysicalOperation(
      ...
      pruneFilterProjectUdfs(
          ...
          (requestedColumns, srcFilters, attr2udf) =>
            toCatalystRDD(l,
                          requestedColumns,
                          t.buildScan(requestedColumns.map {
                            case nat: AttributeReference
                                if attr2udf contains nat.toString =>
                              nat.toString
                            case att => att.name
  ...
  protected def pruneFilterProjectUdfs(
      ...
      scanBuilder: (Seq[Attribute], Array[Filter],
                    Map[String, NativeUDF]) => RDD[InternalRow]) = {
    ...
    val (pro, fil, att2udf) =
      (CatalystToCrossdataAdapter.getConnectorLogicalPlan(
          plan,
          projects,
          filterPredicates): @unchecked) match {
    ...
    val filterSet = AttributeSet(
        filterPredicates.flatMap(
          ...
          case nat: AttributeReference if att2udf contains nat =>
            CatalystToCrossdataAdapter.udfFlattenedActualParameters(
                nat,
                (x: Attribute) => x)(att2udf) :+ nat
    ...
    val scan = execution.PhysicalRDD.createFromDataSource(
        requestedColumns,
        scanBuilder(requestedColumns, fil, att2udf map {
