...
class XDDdlParserSpec extends BaseXDTest with MockitoSugar {
  ...
  it should "successfully parse a INSERT TABLE with qualified table name and more than one VALUES provided into InsertTable RunnableCommand" in {
    ...
    val sentence =
      """INSERT INTO tableId VALUES ( 12, 12.01, 'proof', true), ( 2, 1.01, 'pof', true), ( 256, 0.01, 'pr', false)"""
    parser.parse(sentence) shouldBe
      InsertIntoTable(TableIdentifier("tableId"),
                      List(List("12", "12.01", "proof", "true"),
                           List("2", "1.01", "pof", "true"),
                           List("256", "0.01", "pr", "false")))
  ...
  it should "successfully parse a INSERT TABLE with qualified table name, schema and VALUES provided into InsertTable RunnableCommand" in {
    ...
    val sentence =
      """INSERT INTO tableId(Column1, Column2, Column3, Column4) VALUES ( 256, 0.01, 'pr', false)"""
    parser.parse(sentence) shouldBe
      InsertIntoTable(TableIdentifier("tableId"),
  ...
  it should "successfully parse a INSERT TABLE with qualified table name, schema and more than one VALUES provided into InsertTable RunnableCommand" in {
    ...
    val sentence =
      """INSERT INTO tableId(Column1, Column2, Column3, Column4) VALUES ( 12, 12.01, 'proof', true), ( 2, 1.01, 'pof', true), ( 256, 0.01, 'pr', false)"""
    parser.parse(sentence) shouldBe
      InsertIntoTable(TableIdentifier("tableId"),
                      List(List("12", "12.01", "proof", "true"),
                           List("2", "1.01", "pof", "true"),
  ...
  it should "successfully parse a INSERT TABLE using arrays provided in VALUES" in {
    ...
    val sentence =
      """INSERT INTO tableId VALUES ( [1,2], 12, 12.01, 'proof', [false,true], true, ["proof array", "proof2"])"""
    parser.parse(sentence) shouldBe
      InsertIntoTable(TableIdentifier("tableId"),
                      List(
                          List(List("1", "2"),
  ...
  it should "successfully parse a INSERT TABLE using maps provided in VALUES" in {
    ...
    val sentence =
      """INSERT INTO tableId VALUES ( (x -> 1, y -> 2), 12, 12.01, 'proof', (x1 -> false, x2 -> true), true)"""
    parser.parse(sentence) shouldBe
      InsertIntoTable(TableIdentifier("tableId"),
                      List(
                          List(Map("x" -> "1", "y" -> "2"),
  ...
  it should "successfully parse a INSERT TABLE using maps with strings provided in VALUES" in {
    ...
    val sentence =
      """INSERT INTO tableId VALUES ( (x -> 1, y -> 2, z -> 3), 12, 12.01, 'proof', (x1 -> "proof,comma", x2 -> "proof2"), true)"""
    parser.parse(sentence) shouldBe
      InsertIntoTable(TableIdentifier("tableId"),
                      List(
                          List(Map("x" -> "1", "y" -> "2", "z" -> "3"),
                               "12",
                               "12.01",
                               "proof",
  ...
  it should "successfully parse a CREATE GLOBAL INDEX into a CreateGlobalIndex RunnableCommand" in {
    val sentence = """|CREATE GLOBAL INDEX myIndex
  ...
  it should "successfully parse a CREATE GLOBAL INDEX without USING without dbName into a CreateGlobalIndex RunnableCommand" in {
    val sentence = """|CREATE GLOBAL INDEX myDbIndex.myIndex
         ...
         |)""".stripMargin
    parser.parse(sentence) shouldBe
      CreateGlobalIndex(TableIdentifier("myIndex", Option("myDbIndex")),
