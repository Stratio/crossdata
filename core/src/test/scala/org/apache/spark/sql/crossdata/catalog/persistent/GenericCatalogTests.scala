...
trait GenericCatalogTests extends SharedXDContextTest with CatalogConstants {
  ...
  s"${catalogName}CatalogSpec" must "return a dataframe from a persist table without catalog using json datasource" in {
    ...
    val crossdataTable = CrossdataTable(TableIdentifier(TableName, None).normalize,
                                        Some(Columns),
                                        SourceDatasource,
                                        Array.empty,
                                        opts)
  ...
  it should s"persist a table with catalog and partitionColumns in $catalogName" in {
    ...
    val crossdataTable = CrossdataTable(TableIdentifier(TableName, Some(Database)).normalize,
                                        Some(Columns),
                                        SourceDatasource,
                                        Array(Field1Name),
                                        OptsJSON)
  ...
  it should s"persist a table with catalog and partitionColumns with map with arrays with subdocuments and strange " +
    ...
    df shouldBe a[XDDataFrame]
    df.schema.apply(0).dataType shouldBe (MapType(ColumnsWithSubColumns,
                                                  ColumnsWithArrayWithSubdocuments))
  }

  it should "returns list of tables" in {
    xdContext.catalog.dropAllTables()
    val crossdataTable1 = CrossdataTable(TableIdentifier(TableName, Some(Database)).normalize,
                                         Some(Columns),
                                         SourceDatasource,
                                         Array(Field1Name),
                                         OptsJSON)
    val crossdataTable2 = CrossdataTable(TableIdentifier(TableName, None).normalize,
                                         Some(Columns),
                                         SourceDatasource,
                                         Array(Field1Name),
                                         OptsJSON)
  ...
  it should "persist index in catalog" in {
    ...
    val version = "1.5.0"
    val crossdataIndex =
      CrossdataIndex(tableIdentifier, indexIdentifier, indexedCols, pk, dataSource, opts, version)
