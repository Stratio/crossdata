...
trait SharedXDContextWithDataTest extends SharedXDContextTest with SparkLoggerComponent {
  ...
  def defaultOptions: Map[String, String] =
    Map.empty // Spark options used to register the test table in the catalog

  def sparkRegisterTableSQL: Seq[SparkTable] =
    Nil /* Spark CREATE sentence. Without OPTIONS or USING parts since
  ...
  lazy val assumeEnvironmentIsUpAndRunning = if (!isEnvironmentReady) {
    fail(runningError)
  }
  ...
  implicit def str2sparkTableDesc(query: String): SparkTable =
    SparkTable(query, defaultOptions)
  ...
  protected override def beforeAll(): Unit = {
    ...
    isEnvironmentReady = Try {
      ...
      saveTestData
      sparkRegisterTableSQL.foreach {
        case SparkTable(s, opts) => sql(Sentence(s, provider, opts).toString)
      }
      client.isDefined
    } recover {
      case e: Throwable =>
