...
class ElasticInsertTableIT extends ElasticInsertCollection {
  ...
  it should "insert a row using INSERT INTO table VALUES in ElasticSearch" ignore {
    val query =
      s"""|INSERT INTO $Type VALUES (20, 25, 'proof description', true, 'Eve', false, '2015-01-01' ,
    ...
    //EXPECTATION
    val results = sql(
        s"select id, age, description, enrolled, name, optionalField, birthday, salary, ageInMilis, array_map, map_array from $Type where id=20")
  ...
  it should "insert multiple rows using INSERT INTO table(schema) VALUES in ElasticSearch" ignore {
    _xdContext
      .sql(s"INSERT INTO $Type (age, name, enrolled) VALUES ( 50, 'Samantha', true),( 1, 'Charlie', false)")
