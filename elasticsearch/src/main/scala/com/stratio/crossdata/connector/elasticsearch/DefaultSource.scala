...
class DefaultSource
      ...
      case ErrorIfExists =>
        if (relation.isEmpty()) relation.insert(data, overwrite = false)
          ...
          throw new EsHadoopIllegalStateException(
              s"Index ${relation.cfg.getResourceWrite} already exists")
      case Ignore =>
        if (relation.isEmpty()) {
          relation.insert(data, overwrite = false)
        }
  ...
  private def params(parameters: Map[String, String]) = {
    ...
    val params = parameters.map { case (k, v) => (k.replace('_', '.'), v) }.map {
      case (k, v) =>
    ...
    // validate path
    if (params
          .get(ConfigurationOptions.ES_RESOURCE_READ)
          .orElse(params.get(ConfigurationOptions.ES_RESOURCE))
          .isEmpty)
  ...
  override def createExternalTable(context: SQLContext,
    ...
    val (index, typeName) = ElasticSearchConnectionUtils
      ...
      .orElse(databaseName.map((_, tableName)))
      .getOrElse(throw new RuntimeException(
              s"$ES_RESOURCE is required when running CREATE EXTERNAL TABLE"))
  ...
  override def dropExternalTable(context: SQLContext, options: Map[String, String]): Try[Unit] = {
    ...
    if (ElasticSearchConnectionUtils.numberOfTypes(options) == 1) {
      ...
      sys.error(
          "Cannot remove table from ElasticSearch if more than one table is persisted in the same index. Please remove it natively")
    }
