...
public class Bridge {
     ...
     * 
     * @param session
     *            Cassandra session. {@link com.datastax.driver.core.Session}
     * @param deepSparkContext
     *            Spark context from Deep
     * @param config
     *            A {@link com.stratio.meta.core.engine.EngineConfig}, contains
     *            global configuration
     */
    public Bridge(Session session, DeepSparkContext deepSparkContext,
            EngineConfig config) {
     ...
     * 
     * @param stmt
     *            Statement which corresponds to this node.
     * @param isRoot
     *            Indicates if this node is root in this plan
    ...
    public ResultSet executeLeafNode(MetaStatement stmt, boolean isRoot) {
        ...
        IDeepJobConfig config = DeepJobConfigFactory.create().session(session)
                .host(engineConfig.getRandomCassandraHost())
                .rpcPort(engineConfig.getCassandraPort())
                .keyspace(ss.getKeyspace()).table(ss.getTableName());

        config = (columnsSet.length == 0) ? config.initialize() : config
        ...
        return returnResult(
                rdd,
                isRoot,
                ss.getSelectionClause().getType() == SelectionClause.TYPE_COUNT,
                Arrays.asList(columnsSet));
    }
     ...
     * @param stmt
     *            Statement which corresponds to this node.
     * @param resultsFromChildren
     *            List of results from node children
     * @return a {@link com.stratio.meta.common.data.ResultSet}
     */
    public ResultSet executeRootNode(MetaStatement stmt,
        ...
        for (Result child : resultsFromChildren) {
            ...
            CassandraResultSet crset = (CassandraResultSet) qResult
                    .getResultSet();
        ...
        JavaPairRDD rddRight = rddTableRight.map(new MapKeyForJoin(
                keyTableRight));
     ...
     * @return a {@link com.stratio.meta.common.data.ResultSet}
     */
    public ResultSet execute(MetaStatement stmt,
            List<Result> resultsFromChildren, boolean isRoot) {
     ...
     * 
     * @param rdd
     *            RDD which corresponds to Spark result.
     * @param isRoot
     *            Indicates if this node is root in this plan.
     * @param isCount
     *            Indicates if this query have a COUNT clause.
     * @param selectedCols
     *            List of columns selected in current SelectStatement.
     * @return ResultSet containing the result of built.
     */
    private ResultSet returnResult(JavaRDD rdd, boolean isRoot,
            boolean isCount, List<String> selectedCols) {
        if (isRoot) {
            if (isCount) {
                return DeepUtils.buildCountResult(rdd);
            }
            return DeepUtils.buildResultSet(rdd.take(DEFAULT_RESULT_SIZE),
                    selectedCols);
        } else {
            CassandraResultSet crs = new CassandraResultSet();
    ...
    /**
     * Take a RDD and a Relation and apply suitable filter to the RDD. Execute
     * where clause on Deep.
     * 
     * @param rdd
     *            RDD which filter must be applied.
     * @param rel
     *            {@link com.stratio.meta.core.structures.Relation} to apply
    ...
    /**
     * Take {@link com.stratio.meta.deep.Bridge#DEFAULT_RESULT_SIZE} from RDD
     * ordered.
     * 
     * @param rdd
     *            RDD to take and order.
     * @param orderings
     *            Order By clause.
     * @return RDD result.
     */
    public JavaRDD doOrder(JavaRDD rdd, List<Ordering> orderings) {
        List<Cells> list = rdd.takeOrdered(DEFAULT_RESULT_SIZE,
                new DeepComparator(orderings));
