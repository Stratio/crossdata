...
class MongoQueryProcessorSpec extends BaseXDTest {
  ...
  "A MongoQueryProcessor" should "build a query requiring some columns" in {
    val (filters, requiredColumns) = MongoQueryProcessor
      .buildNativeQuery(Array(ColumnId, ColumnAge), Array(), config)
  ...
  it should "build a query with two equal filters" in {
    val (filters, requiredColumns) = MongoQueryProcessor.buildNativeQuery(
        Array(ColumnId),
        Array(EqualTo(ColumnAge, ValueAge), EqualTo(ColumnId, ValueId)),
        config)
  ...
  it should "build a query with an IN clause" in {
    val (filters, requiredColumns) = MongoQueryProcessor.buildNativeQuery(
        Array(ColumnId),
        Array(In(ColumnAge, Array(ValueAge, ValueAge2))),
        config)
    val filterSet = filters.keySet
    ...
    val inListValues =
      filters.get(ColumnAge).asInstanceOf[DBObject].get(QueryOperators.IN)
    inListValues should matchPattern { case _: Array[_] => }
    inListValues.asInstanceOf[Array[Any]] should have size 2
    inListValues
      .asInstanceOf[Array[Any]] should contain allOf (ValueAge, ValueAge2)
  ...
  it should "build a query with a LT clause" in {
    val (filters, requiredColumns) = MongoQueryProcessor.buildNativeQuery(
        Array(ColumnId),
        Array(LessThan(ColumnAge, ValueAge)),
        config)
    val filterSet = filters.keySet
    ...
    filters
      ...
      .get(QueryOperators.LT) shouldBe (ValueAge)
  ...
  it should "build a query with a LTE clause" in {
    val (filters, requiredColumns) = MongoQueryProcessor.buildNativeQuery(
        Array(ColumnId),
        Array(LessThanOrEqual(ColumnAge, ValueAge)),
        config)
    val filterSet = filters.keySet
    ...
    filters
      .get(ColumnAge)
      .asInstanceOf[DBObject]
      .get(QueryOperators.LTE) shouldBe (ValueAge)
  ...
  it should "build a query with a GTE clause" in {
    val (filters, requiredColumns) = MongoQueryProcessor.buildNativeQuery(
        Array(ColumnId),
        Array(GreaterThanOrEqual(ColumnAge, ValueAge)),
        config)
    val filterSet = filters.keySet
    ...
    filters
      .get(ColumnAge)
      .asInstanceOf[DBObject]
      .get(QueryOperators.GTE) shouldBe (ValueAge)

  }

  it should "build a query with an IS NOT NULL clause" in {
    val (filters, requiredColumns) = MongoQueryProcessor
      .buildNativeQuery(Array(ColumnId), Array(IsNotNull(ColumnAge)), config)
    val filterSet = filters.keySet
    ...
    filters
      .get(ColumnAge)
      .asInstanceOf[DBObject]
      .get(QueryOperators.NE) shouldBe (null)
  ...
  it should "build a query with an AND(v1 > x <= v2)" in {
    val (filters, requiredColumns) = MongoQueryProcessor.buildNativeQuery(
        Array(ColumnId),
        Array(
            And(GreaterThan(ColumnAge, ValueAge),
                LessThanOrEqual(ColumnAge, ValueAge2))),
        config)
    val filterSet = filters.keySet
    ...
    val subfilters =
      filters.get(QueryOperators.AND).asInstanceOf[util.ArrayList[DBObject]]
    ...
    subfilters
      ...
      .get(ColumnAge)
      .asInstanceOf[DBObject]
      .get(QueryOperators.GT) shouldBe (ValueAge)
    ...
    subfilters
      ...
      .get(ColumnAge)
      .asInstanceOf[DBObject]
      .get(QueryOperators.LTE) shouldBe (ValueAge2)
  }
  ...
  it should "build a query with a REGEX clause " in {
    ...
    val (filters, requiredColumns) = MongoQueryProcessor.buildNativeQuery(
        Array(ColumnId),
        Array(StringContains(ColumnId, ValueId.toString)),
        config)
    val filterSet = filters.keySet
    ...
    filters.get(ColumnId).asInstanceOf[Pattern].pattern should be(
        Pattern.compile(s".*${ValueId.toString}.*").pattern)
  }
