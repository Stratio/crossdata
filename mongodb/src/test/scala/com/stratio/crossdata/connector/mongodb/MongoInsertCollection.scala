...
trait MongoInsertCollection extends MongoWithSharedContext {
  ...
  override def sparkRegisterTableSQL: Seq[SparkTable] =
    super.sparkRegisterTableSQL :+
      str2sparkTableDesc(
          s"""|CREATE TEMPORARY TABLE $Collection (id BIGINT, age INT, description STRING, enrolled BOOLEAN,
  ...
  override def defaultOptions =
    super.defaultOptions + ("collection" -> s"$Collection")
}
