...
class MongoInsertTableIT extends MongoInsertCollection {
  ...
  it should "insert a row using INSERT INTO table(schema) VALUES in MongoDb" in {
    ...
    //EXPECTATION
    val results = sql(s"select age, name, enrolled from $Collection where age=25 and name='Peter'").collect()
  ...
  it should "insert multiple rows using INSERT INTO table VALUES in MongoDb" in {
    ...
    results should contain allOf(
      ...
      Row(21, 25, "proof description", true, "John", false, Seq("4", "5"),
        Map("x" -> "1"), Seq(Map("z" -> "1")), Map("x" -> Seq("1", "2"))),

      Row(22, 1, "other description", false, "James", true, Seq("1", "2", "3"),
        Map("key" -> "value"), Seq(Map("a" -> "1")), Map("x" -> Seq("1", "a"))),

      Row(23, 33, "other fun description", false, "July", false, Seq("true", "true"),
        Map("z" -> "1", "a" -> "2"), Seq(Map("za" -> "12")), Map("x" -> Seq("1", "2")))
      )
  ...
  it should "insert multiple rows using INSERT INTO table(schema) VALUES in MongoDb" in {
    ...
    //EXPECTATION
    val results = sql(s"select age, enrolled, name from $Collection where (age=50 and name='Samantha') or (age=1 and name='Charlie')").collect()
  ...
  it should "insert rows using INSERT INTO table(schema) VALUES with Arrays in MongoDb" in {
    ...
    //EXPECTATION
    val results = sql(s"select age, name, enrolled, array_test from $Collection where (age=55 and name='Jules') or (age=12 and name='Martha')").collect()
  ...
  it should "insert rows using INSERT INTO table(schema) VALUES with Map in MongoDb" in {
    ...
    //EXPECTATION
    val results = sql(s"select age, name, enrolled, map_test from $Collection where (age=12 and name='Albert') or (age=20 and name='Alfred')").collect()
  ...
  it should "insert rows using INSERT INTO table(schema) VALUES with Array of Maps in MongoDb" in {
    ...
    //EXPECTATION
    val results = sql(s"select age, name, enrolled, array_map from $Collection where (age=1 and name='Nikolai') or (age=14 and name='Ludwig')").collect()
  ...
  it should "insert rows using INSERT INTO table(schema) VALUES with Map of Array in MongoDb" in {
    ...
    //EXPECTATION
    val results = sql(s"select age, name, enrolled, map_array from $Collection where (age=13 and name='Svletiana') or (age=17 and name='Wolfang')").collect()
