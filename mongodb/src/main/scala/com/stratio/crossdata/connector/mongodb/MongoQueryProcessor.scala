...
object MongoQueryProcessor {
  ...
  def filtersToDBObject(
    ...
    sFilters.foreach {
      ...
      case sources.GreaterThanOrEqual(attribute, value) =>
        queryBuilder
          .put(attstr2left(attribute))
          .greaterThanEquals(correctIdValue(attribute, value))
  ...
  private def selectFields(fields: Seq[ColumnName]): DBObject = {
    MongoDBObject(fields.toList.filterNot(_ == "_id").map(_ -> 1) ::: {
...
// TODO logs, doc, tests
class MongoQueryProcessor(logicalPlan: LogicalPlan,
                          config: Config,
                          schemaProvided: Option[StructType] = None)
  ...
  def execute(): Option[Array[Row]] = {
    ...
    if (schemaProvided.isEmpty) {
      ...
      try {
        validatedNativePlan.map {
          case MongoPlan(bs: SimpleLogicalPlan, limit) =>
            if (limit.exists(_ == 0)) {
              Array.empty[Row]
            } else {
  ...
  def validatedNativePlan: Option[_] = { // TODO
    lazy val limit: Option[Int] = logicalPlan.collectFirst {
      case LogicalLimit(Literal(num: Int, _), _) => num
    }
    ...
    findBasePlan(logicalPlan).collect {
      case bp if checkNativeFilters(bp.filters) => MongoPlan(bp, limit)
    }
  }

  private[this] def checkNativeFilters(filters: Seq[SourceFilter]): Boolean =
    filters.forall {
