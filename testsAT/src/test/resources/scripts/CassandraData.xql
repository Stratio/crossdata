...
CREATE FULL_TEXT INDEX catalogTest.tableTest.name_full_text_index ON catalogTest.tableTest(catalogTest.tableTest.name);
CREATE DEFAULT INDEX catalogTest.tableTest.age_default_index ON catalogTest.tableTest(catalogTest.tableTest.age);
...
CREATE TABLE IF NOT EXISTS catalogTest.tab1 ON CLUSTER ClusterTest(id int, age int, PRIMARY KEY(id));
CREATE DEFAULT INDEX catalogTest.tab1.age_index ON catalogTest.tab1(catalogTest.tab1.age);
...
CREATE TABLE IF NOT EXISTS catalogTest.tab2 ON CLUSTER ClusterTest(name text, surname text, PRIMARY KEY(name));
CREATE DEFAULT INDEX catalogTest.tab2.surname_index ON catalogTest.tab2(catalogTest.tab2.surname);
...
CREATE TABLE IF NOT EXISTS catalogTest.tab3 ON CLUSTER ClusterTest(id BIGINT, age BIGINT, PRIMARY KEY(id));
CREATE DEFAULT INDEX catalogTest.tab3.age_index_bigint ON catalogTest.tab3(catalogTest.tab3.age);
...
CREATE TABLE IF NOT EXISTS catalogTest.tab4 ON CLUSTER ClusterTest(money DOUBLE, reten DOUBLE, PRIMARY KEY(money));
CREATE DEFAULT INDEX catalogTest.tab4.reten_index ON catalogTest.tab4(catalogTest.tab4.reten);
...
CREATE TABLE IF NOT EXISTS catalogTest.tab5 ON CLUSTER ClusterTest(money FLOAT, reten FLOAT, PRIMARY KEY(money));
CREATE DEFAULT INDEX catalogTest.tab5.reten_index_float ON catalogTest.tab5(catalogTest.tab5.reten);
...
CREATE TABLE IF NOT EXISTS catalogTest.tab6 ON CLUSTER ClusterTest(married BOOLEAN , new BOOLEAN,  PRIMARY KEY(married));
CREATE DEFAULT INDEX catalogTest.tab6.new_index ON catalogTest.tab6(catalogTest.tab6.new);
INSERT INTO catalogTest.tab6(married, new) VALUES (true, false);
INSERT INTO catalogTest.tab6(married, new) VALUES (false, true);

CREATE TABLE catalogTest.usersorderby ON CLUSTER ClusterTest ( company text, client text, factcompany bigint, factclient double, futurefact float, foundationyear int, stockmarket boolean, PRIMARY KEY (company, client));
