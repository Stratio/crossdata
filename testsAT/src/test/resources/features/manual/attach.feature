...
Feature: Test crossdata shell attach/add operations
  ...
  Scenario: Use empty options
    Given I run the shell command "ATTACH CLUSTER exampleCluster ON DATASTORE Cassandra WITH OPTIONS {};"
    Then I expect a 'Some required properties are missing' message

  Scenario: Use invalid options
    Given I run the shell command "ATTACH CLUSTER exampleCluster ON DATASTORE Cassandra WITH OPTIONS {'myOption': 'myValue'};"
    Then I expect a 'Some required properties are missing' message
  ...
  Scenario: Use empty cluster
    Given I run the shell command "ATTACH CONNECTOR CassandraConnector TO WITH OPTIONS {'DefaultLimit': '1000'};"
    Then I expect a 'Parser exception' message
  ...
  Scenario: Use invalid options
    Given I run the shell command "ATTACH CONNECTOR CassandraConnector TO testCluster WITH OPTIONS {'invalidOption': 'invalidValue'};"
    Then I expect a 'Some required properties are missing' message
