Feature: Basic Select queries with FILTEr_PK_GT
  ...
  Scenario: SELECT id FROM catalogTest.tableTest WHERE id > 5;
    When I execute a query: 'SELECT id FROM catalogTest.tableTest WHERE id > 5;'
    Then the result has to be:
  ...
  Scenario: SELECT * AS identificador FROM catalogTest.tab2 WHERE name > 'Carlos';
    When I execute a query: 'SELECT * FROM catalogTest.tab2 WHERE name > 'Carlos';'
    Then the result has to be:
  ...
  Scenario: SELECT * AS identificador FROM catalogTest.tab2 WHERE name > 'AAAA';
    When I execute a query: 'SELECT * FROM catalogTest.tab2 WHERE name > 'AAAA';'
    Then the result has to be:
      | catalogTest.tab2.name-name-String | catalogTest.tab2.surname-surname-String |
      | Antonio                           | Alcocer                                 |
      | Hugo                              | Dominguez                               |
  ...
  Scenario: SELECT * FROM catalogTest.tab3 WHERE id > 0;
    When I execute a query: 'SELECT * FROM catalogTest.tab3 WHERE id > -4.5;'
    Then the result has to be:
      | catalogTest.tab3.id-id-BigInteger | catalogTest.tab3.age-age-BigInteger |
      | -4                                | -40                                 |
      | 1                                 | 10                                  |
