...
public class CukesHooks extends BaseSpec implements ICucumberReporter, ICucumberFormatter {
    ...
    @Override
    public void eof() {
        commonspec.getMetaDriver().close();
        commonspec.getLogger().info("Disconected to MetaServer");
        logger.info("The feature ends");
    }
    ...
    public void feature(Feature feature) {
        ...
        if (ThreadProperty.get("ExecutionType").equals("Crossdata")) {
            commonspec.getLogger().info("THROUGH CROSSDATA");
            try {
                commonspec.getMetaDriver().connect("");

            } catch (ConnectionException e) {
                commonspec.getLogger().error("Imposible to connect");
            }
        }else{
            try {
                commonspec.getJDBCDriver().connect();
            } catch (ClassNotFoundException e) {
                commonspec.getLogger().error("Imposible to connect: " + e.toString());
                e.printStackTrace();
...
//        commonspec.getMetaDriver().cleanMetadata();
    }
    ...
    public void result(Result result) {
        logger.info("Done executing {}: {}", this.scenario.getName(), result.getStatus());
    }
