...
class DriverIT extends EndToEndTest {
  ...
  "CrossdataDriver" should "return an ErrorResult when running an unparseable query" in {
    ...
    withDriverDo { driver =>
      ...
      result
        .asInstanceOf[ErrorSQLResult]
        .cause
        .get
        .getMessage should include regex "cannot resolve .*"
    }
  }
  ...
  it should "be able to execute ADD APP Command of an existent file with options" in {
    ...
    withDriverDo { driver =>
      ...
      val executeResult = driver
        .sql("""EXECUTE testApp(rain,bow2) OPTIONS (executor.memory '20G')""")
        .waitForResult()
