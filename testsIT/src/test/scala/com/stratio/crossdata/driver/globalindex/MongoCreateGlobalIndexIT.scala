...
class MongoCreateGlobalIndexIT extends MongoAndElasticWithSharedContext {
  ...
  protected override def beforeAll(): Unit = {
    ...
    mongoClient(mongoTestDatabase)("proofGlobalIndex").insert(
        MongoDBObject("id" -> 13,
                      "name" -> "prueba2",
                      "comments" -> "one comment fail",
                      "other" -> 5,
                      "another" -> 12)
    )

    val sentence = s"""|CREATE GLOBAL INDEX myIndex
    ...
    elasticClient.execute {
      index into "gidx" / "myIndex" fields ("id" -> 11,
          "another" -> 12,
          "other" -> 12)
    }.await

    elasticClient.execute {
      index into "gidx" / "myIndex" fields ("id" -> 13,
          "another" -> 12,
          "other" -> 5)
    }.await
  ...
  it should "execute a select col where indexedFilter equals to using multiple projects via DDL" in {
    val result = xdContext
      .table("globalIndexDb.proofGlobalIndex")
      .select("name", "other")
      .where($"other" equalTo 5)
      .select("name")
      .collect()
    result should have length 1
    result shouldBe Array(Row("prueba2"))
  ...
  it should "support filters mixed with indexedCols" in {
    val result = sql(
        s"select name from globalIndexDb.proofGlobalIndex WHERE other > 10 AND name LIKE '%prueba%'")
      .collect()
