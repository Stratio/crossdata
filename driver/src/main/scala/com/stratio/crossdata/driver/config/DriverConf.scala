...
class DriverConf extends Logging {
  ...
  private[crossdata] lazy val finalSettings: Config = userSettings.foldLeft(typesafeConf) {
  ...
  def setTunnelTimeout(seconds: Int): DriverConf = {
    userSettings
      .put(AkkaClusterRecepcionistTunnelTimeout, ConfigValueFactory.fromAnyRef(seconds * 1000))
    this
  }
  ...
  private val typesafeConf: Config = {
    ...
    val finalConfigWithEnvVars = {
      if (finalConfigWithSystemProperties.hasPath("config.cluster.servers")) {
        ...
        defaultConfig.withValue(DriverConfigHosts,
