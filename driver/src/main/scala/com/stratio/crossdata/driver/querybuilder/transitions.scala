...
trait Groupable {
  ...
  def groupBy(groupingExpressions: String): GroupedQuery =
...
trait Sortable {
  ...
  def sortBy(ordering: SortOrder*): SortedQuery =
    orderOrSortBy(global = false, ordering)
...
trait Limitable {
  ...
  def limit(value: Int): LimitedQuery =
    new LimitedQuery(context,
                     projections,
                     relation,
                     filters,
                     groupingExpressions,
...
trait Combinable extends CrossdataSQLStatement {
  ...
  private def computeCombinationInfo( //TODO: Simplify this
    ...
    composition map {
      case CombinationInfo(combType, previous) =>
        CombinationInfo(combType, childCombination(previous))
    } getOrElse {
